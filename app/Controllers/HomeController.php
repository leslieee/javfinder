    ...
    public function watch($request, $response, $args)
    {
        ...
        if ($info->embed == "" || $info->embed == "null") {
            ...
            curl_setopt_array($curl, array(
                CURLOPT_URL => Config::get('source_host') . "/stream/sw0/" . $id,
            ...
            if ($err) {
                ...
                $info->save();
                curl_setopt_array($curl, array(
                    CURLOPT_URL => Config::get('embed_host') . "/api/source/" . $out,
                    CURLOPT_RETURNTRANSFER => true,
                    CURLOPT_ENCODING => "",
                    CURLOPT_MAXREDIRS => 10,
                    CURLOPT_TIMEOUT => 30,
                ...
                if ($err) {
                    ...
                    $last = end($res["data"]);
                    $url  = Config::get('embed_host') . $last["file"];
                    // 解析真实地址
                    $ch = curl_init();
            ...
            $curl = curl_init();
            curl_setopt_array($curl, array(
                CURLOPT_URL => Config::get('embed_host') . "/api/source/" . $info->embed,
                CURLOPT_RETURNTRANSFER => true,
                CURLOPT_ENCODING => "",
                CURLOPT_MAXREDIRS => 10,
                CURLOPT_TIMEOUT => 30,
